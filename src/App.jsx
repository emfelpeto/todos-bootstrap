...
export default function App() {
   ...
   return (
      <div className='container-sm border border-secondary rounded p-3 mt-5'>
         ...
         {todos.length === 0 && (
            <div className="d-flex column-gap-2 align-items-center justify-content-center">
               <div className="spinner-border text-warning spinner-border-sm" role="status">
                  <span className="visually-hidden">Loading...</span>
               </div>
               <span className="text-warning-emphasis">Retrieven data</span>
            </div>
            )}
         ...
         { /* TODO LIST */
            ...
            todo => !todo.completed).length === 0 || (
               <div className='todo-list-container row gy-2 mx-0'> {/* CONDITIONAL WITH || */}
                  ...
                  {todos.map((todo) => (
                     !todo.completed && <TodoItem key={todo.id} todo={todo} handleComplete={handleComplete} handleDelete={handleDelete}/>
                  ))}
               </div>
            )
         }
         ...
         todos.filter(todo => todo.completed).length !== 0 && (
            <div className='todo-completed-container row gy-2 mx-0 mt-3'> {/* SAME CONDITIONAL BUT WITH && */}
               <div className='d-flex align-items-center justify-content-between'>
